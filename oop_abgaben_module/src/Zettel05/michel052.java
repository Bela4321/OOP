...
public class michel052 {
    ...
    double maxVelocity(double[] gps) {
        ...
        for (int i = 0; i < gps.length; i = i + 3) {
            max = Math.sqrt(Math.pow(gps[gps.length - 2] - gps[0], 2) + Math.pow(gps[(gps.length) - 1] - gps[1], 2) + Math.pow(gps[gps.length] - gps[2], 2)) / gps.length;
...
// hier fehlt noch die letzte 5.2.
